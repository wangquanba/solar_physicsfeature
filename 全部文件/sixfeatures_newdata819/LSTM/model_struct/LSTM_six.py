...
if __name__ == '__main__':
            ...
            for x in range(0, 10):
                ...
                num5 = 7
                num6 = 8
                x_train, y_train = data_process(train_df, [num1, num2, num3, num4, num5,num6])
                x_valid, y_valid = data_process(valid_df, [num1, num2, num3, num4, num5,num6])
                x_test, y_test = data_process(test_df, [num1, num2, num3, num4, num5,num6])
                # print(x_train)
                # print(x_valid)
                # print(y_valid)
                # print(x_test)
                ...
                Accuracy = m.Accuracy()
                print(
                    f"{x},数据特征个数为{6}，数据特征为{feature_name[num1 - 3]}+{feature_name[num2 - 3]}+{feature_name[num3 - 3]}+{feature_name[num4 - 3]}+{feature_name[num5 - 3]}+{feature_name[num6 - 3]}，"
                    f"TSS最大值：{m.TSS()}，HSS最大值：{m.HSS()}，Accuracy最大值:{m.Accuracy()}")
                data.append(
                    [x, feature_name[num1 - 3]+feature_name[num2 - 3]+feature_name[num3 - 3]+feature_name[num4 - 3]+feature_name[num5 - 3]+feature_name[num6 - 3], TSS[0], HSS[0],
                     Accuracy[0]])
            ...
            plt.ylabel('Loss')
            plt.legend()
            plt.savefig('../figure/train_loss_figure/LSTM_6train.png')  # 保存位置
            # plt.show()
            # 绘制验证loss
            plt.figure(dpi=600)
            for i, history in enumerate(all_loss):

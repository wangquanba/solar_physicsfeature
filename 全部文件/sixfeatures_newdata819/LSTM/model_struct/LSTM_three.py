...
if __name__ == '__main__':
                    ...
                    for p in range(4, 9):
                        if p <= 8 and p > i:
                            ...
                            for z in range(5, 9):
                                if z <= 8 and z > p:
                                    num3 = z
                                    x_train, y_train = data_process(train_df, [num1, num2, num3])
                                    x_valid, y_valid = data_process(valid_df, [num1, num2, num3])
                                    x_test, y_test = data_process(test_df, [num1, num2, num3])
                                    # print(x_train)
                                    # print(x_valid)
                                    # print(y_valid)
                                    # print(x_test)
                                    ...
                                    best_model = "../save_model/model_threeclass" + str(
                                        x) + ".h5"
                                    ...
                                    Accuracy = m.Accuracy()
                                    print(
                                        f"{x},数据特征个数为{3}，数据特征为{feature_name[num1 - 3]}+{feature_name[num2 - 3]}+{feature_name[num3 - 3]}，"
                                        f"TSS最大值：{m.TSS()}，HSS最大值：{m.HSS()}，Accuracy最大值:{m.Accuracy()}")
                                    data.append(
                                        [x, feature_name[num1 - 3]+feature_name[num2 - 3]+feature_name[num3 - 3], TSS[0], HSS[0],
                                         Accuracy[0]])
            ...
            save_data.to_csv('../test_result/LSTM_feature3.csv')

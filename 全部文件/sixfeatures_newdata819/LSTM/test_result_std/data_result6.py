...
data = pd.read_csv(fr'../test_result/LSTM_feature6.csv', header=0)
...
feature_set = feature[0:1]
# 几个数据集，然后找到所有的TSS\HSS\Accuracy
TSS_data = []
HSS_data = []
Accuracy_data = []
# TSS_total = 0
...
TSS_data = TSS_data.reshape(1, 10)
HSS_data = np.array(HSS_data)
HSS_data = HSS_data.reshape(1, 10)
Accuracy_data = np.array(Accuracy_data)
Accuracy_data = Accuracy_data.reshape(1, 10)
# print(Accuracy_mean)
...
save_result_df = pd.DataFrame(save_result, index=None,columns = ['feature','TSS_mean', 'TSS_std', 'HSS_mean', 'HSS_std', 'Accuracy_mean',
           'Accuracy_std'])
save_result_df.to_csv('./result_std/LSTM6.csv')
